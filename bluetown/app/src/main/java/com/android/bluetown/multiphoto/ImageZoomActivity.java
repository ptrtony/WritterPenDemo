...
public class ImageZoomActivity extends TitleBarActivity implements
		OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addContentView(R.layout.act_zoom);
	...
	public void initTitle() {
		...
		setBackImageView();
		setTitleView(R.string.image_scan);
		setTitleLayoutBg(R.color.title_bg_blue);
		rightImageLayout.setVisibility(View.INVISIBLE);
	}

	private void initData() {
		differentiate = getIntent().getStringExtra("differentiate");
		currentPosition = getIntent().getIntExtra(
				IntentConstants.EXTRA_CURRENT_IMG_POSITION, 0);
		if (differentiate.equals("circle")) {
			mDataList = PublishPostActivity.mDataList;
		}
	...
	class MyPageAdapter extends PagerAdapter {
		...
		public MyPageAdapter(List<String> dataList) {
			...
			for (int i = 0; i != size; i++) {
				...
				finalBitmap.display(iv, dataList.get(i));
