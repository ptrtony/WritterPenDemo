...
public class DeviceUtil {
	 ...
	 * 但如果两个手机应用了同样的硬件以及Rom镜像（ROM版本、制造商、CPU型号
	 * 、以及其他硬件信息），那计算的ID就不是唯一的，出现此类情况一般可以忽略。
	 * 
	 * @return
	 */
	...
	public static String toUUIDUniqueId(Context context) {
		UUID deviceUuid = new UUID(getAndroidId(context).hashCode(),
				((long) getIMEIDeviceId(context).hashCode() << 32)
						| getSimSerialNumber(context).hashCode());
