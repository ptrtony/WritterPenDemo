 ...
 * 
 */
package com.android.bluetown.monitor.resource;
 ...
 * @Description:TODO(获取资源逻辑控制类)   
 * @author: shenyz   
 * @date:   2015年9月10日 下午3:02:34   
 *
 */
public final class ResourceControl {
    ...
    public void reqResList(int pType, int pId) {
        switch (pType) {
            case MonitorConstant.Resource.TYPE_UNKNOWN:// 第一次请求资源列表
                requestFirstList();
            break;
            case MonitorConstant.Resource.TYPE_CTRL_UNIT:// 从控制中心获取子资源列表
                requestSubResFromCtrlUnit(pId);
            break;
            case MonitorConstant.Resource.TYPE_REGION:// 从区域获取子资源列表
    ...
    private void requestSubResFromRegion(int pId) {
        ...
        if(loginData == null){
            Log.i(MonitorConstant.LOG_TAG, "getRegionListFromRegion loginData : " + loginData);
            return;
        }
        ...
        // 1.从区域获取区域列表
        boolean ret = VMSNetSDK.getInstance().getRegionListFromRegion(servAddr, loginData.sessionID, pId, numPerPage,
                curPage, regionList);
        Log.i(MonitorConstant.LOG_TAG, "getRegionListFromRegion ret : " + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(MonitorConstant.LOG_TAG, "Invoke VMSNetSDK.getRegionListFromRegion failed:" + errDesc());
        }

        List<CameraInfo> cameraList = new ArrayList<CameraInfo>();
        // 2.从区域获取监控点（摄像头）列表
        ret = VMSNetSDK.getInstance().getCameraListFromRegion(servAddr, loginData.sessionID, pId, numPerPage, curPage,
                cameraList);
        Log.i(MonitorConstant.LOG_TAG, "getCameraListFromRegion ret : " + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(MonitorConstant.LOG_TAG, "Invoke VMSNetSDK.getCameraListFromRegion failed:" + errDesc());
        }
    ...
    private void requestSubResFromCtrlUnit(int pId) {
        ...
        ServInfo loginData = TempData.getIns().getLoginData();
        if(loginData == null){
            Log.i(MonitorConstant.LOG_TAG, "requestSubResFromCtrlUnit loginData:" + loginData);
            return;
        }
        ...
        Log.i(MonitorConstant.LOG_TAG, "getControlUnitList ret:" + ret);
        responseFlag = responseFlag || ret;

        if (!ret) {
            Log.e(MonitorConstant.LOG_TAG, "Invoke VMSNetSDK.getControlUnitList failed:" + errDesc());
        }
        List<RegionInfo> regionList = new ArrayList<RegionInfo>();
        // 2.从控制中心获取区域列表
        ret = VMSNetSDK.getInstance().getRegionListFromCtrlUnit(servAddr, sessionID, controlUnitID, numPerPage,
                curPage, regionList);
        Log.i(MonitorConstant.LOG_TAG, "getRegionListFromCtrlUnit ret:" + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(MonitorConstant.LOG_TAG, "Invoke VMSNetSDK.getRegionListFromCtrlUnit failed:" + errDesc());
        }

        List<CameraInfo> cameraList = new ArrayList<CameraInfo>();
        // 3.从控制中心获取摄像头列表
        ret = VMSNetSDK.getInstance().getCameraListFromCtrlUnit(servAddr, sessionID, controlUnitID, numPerPage,
                curPage, cameraList);
        Log.i(MonitorConstant.LOG_TAG, "getCameraListFromCtrlUnit ret:" + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(MonitorConstant.LOG_TAG, "Invoke VMSNetSDK.getCameraListFromCtrlUnit failed:" + errDesc());
        }
        List<Object> allData = new ArrayList<Object>();
        allData.addAll(ctrlUnitList);
        allData.addAll(regionList);
        allData.addAll(cameraList);
        Log.i(MonitorConstant.LOG_TAG, "allData size is " + allData.size());
        if (callback != null) {
    ...
    private void requestFirstList() {
        ...
        ServInfo loginData = TempData.getIns().getLoginData();
        if (loginData == null) {
            Log.i(MonitorConstant.LOG_TAG, "requestFirstList loginData:" + loginData);
            return;
        }
        String sessionID = loginData.sessionID;
        ...
        if (ctrlUnitList != null && !ctrlUnitList.isEmpty()) {
            for (ControlUnitInfo info : ctrlUnitList) {
                Log.i(MonitorConstant.LOG_TAG, "name:" + info.name + ",controlUnitID:" + info.controlUnitID + ",parentID:"
                        + info.parentID);
            }
        }
        Log.i(MonitorConstant.LOG_TAG, "allData size is " + ctrlUnitList.size());
        if (!ret) {
            Log.e(MonitorConstant.LOG_TAG, "Invoke VMSNetSDK.getControlUnitList failed:" + errDesc());
        }
        if (callback != null) {
